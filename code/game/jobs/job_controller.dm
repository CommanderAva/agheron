var/global/datum/controller/occupations/SSjobs
...
/datum/controller/occupations/proc/allow_one_more(var/rank)	//making additional slot on the fly regardless of whether or not the current slots are full.
	var/datum/job/job = GetJob(rank)
	if(job)
		job.total_positions++
